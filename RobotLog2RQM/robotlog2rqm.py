...
**Returns:**
   ...
   """
   cmdlineparser=argparse.ArgumentParser(prog="RobotLog2RQM (RobotXMLResult to RQM importer)", 
                                       description="RobotLog2RQM imports XML result files (default: output.xml) " + \
                                                   "generated by the Robot Framework into a IBM Rational Quality Manager."
                                       )

   cmdlineparser.add_argument('-v',action='version', version=f'v{VERSION} ({VERSION_DATE})',help='Version of the RobotLog2RQM importer.')
   cmdlineparser.add_argument('resultxmlfile', type=str, help='absolute or relative path to the xml result file or directory of result files to be imported.')
...
(*no returns*)
   ...
   if os.path.exists(args.resultxmlfile):
      ...
      Logger.log_error("xml result file is not existing: '%s'" % str(args.resultxmlfile), fatal_error=True)
   ...
   if sLogFileType=="FILE":
      ...
      if len(listEntries) == 0:
         Logger.log_error("No result file under '%s' folder." % str(args.resultxmlfile), fatal_error=True)
